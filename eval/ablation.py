...
CONFIGS = {
    ...
    "- turn-1 tail capture":  {"turn1_tail": False},
    "- ask rotation":         {"rotate_asks": False},
...
Field reweighting is the weakest transfer among those four, worth 0.009 on the
public set and 0.004 on synthetic. It is also the most heavily tuned component (a
six-point weight sweep) and the least mechanistically explained, so it is a
plausible locus of whatever fitting exists. It remains positive on both sets, so
there is nothing to correct, but we treat its public-set contribution as an
overestimate.
...
**Exact rescoring is our weakest transfer overall, and we flag it as such.** The
three draws above were re-run with the component enabled. It is worth +0.0227 on
the public set but only +0.0111 on the synthetic mean, and per draw the spread is
wide: -0.0016, +0.0254, +0.0095. Standard deviation across draws rose from 0.0150
to 0.0285, and the gap from public widened from 0.034 to 0.046.

The mechanism explains the pattern. Rescoring requires the disclosed phrase to
appear as a literal substring of the flattened features/details text. Curated
products have well-populated fields where it reliably does; records drawn at
random from the catalog include sparse and dict-shaped entries where the
flattening does not reproduce the constraint string exactly. It also reorders
only the top 20, so it cannot rescue a target that retrieval placed below that.

We expected better than this. The reasoning was that the component depends only
on the verbatim property that defines the benchmark rather than on any tuned
parameter, so it should transfer at least as well as the components above. That
ignored the field-population difference between curated and uncurated records —
which the category-filter result had already flagged in this same section.

We retain it: it is positive on the mean and on two of three draws, and the
private 800 come from the same curated pipeline as the public 200, so they should
resemble the population where it works. But it is the component we would expect
to contribute least on unseen data, and the public-set figure of +0.023 should be
read as an upper bound rather than an expectation.
"""
...
| Measure                              | Value    |
|--------------------------------------|----------|
| Median per-turn latency              | 11.0 ms  |
| Mean per-turn latency                | 16.8 ms  |
| p95 per-turn latency                 | 50.1 ms  |
| p99 per-turn latency                 | 89.5 ms  |
| Worst single turn                    | 147.7 ms |
| Full 200-session run, after startup  | 8.6 s    |
| FTS5 index build (once per process)  | 34.7 s   |
| Peak RSS                             | 443.4 MB |
| ...baseline before agent constructed | 236.1 MB |
| ...marginal cost of the agent        | ~207 MB  |
| LLM tokens consumed                  | 0        |

**These are the only figures in this document that vary between runs.** A run
taken while the machine was under load reported a 30.6 ms median and a 69 s index
build — roughly three times the table above, from identical code. Wall-clock
timing depends on system state. The scores do not: the agent is deterministic, so
a change in score is always a real effect of a code change, never variance. We
report the unloaded run as representative and disclose the range rather than
picking whichever number reads better.

**Exact-substring rescoring costs memory, not time.** Per-turn latency is
unchanged from before the component was added — 10.9 ms median then, 11.0 ms now.
A substring scan over 20 candidates is negligible beside the FTS query. But
Python-level allocation rose from 10.4 MB to 52.9 MB, and peak RSS from 386 MB to
443 MB. That is the cache of lowercased `features` + `details` text for all
50,000 products, built once at construction. The component is close to free at
query time and costs roughly 57 MB resident.

Two figures deserve qualification rather than a favourable reading.

**The peak RSS is not all ours.** 236.1 MB is already resident before the agent
is constructed — the Python interpreter plus the evaluator's own in-memory
catalog. The agent's marginal footprint is roughly 207 MB: the SQLite FTS5 index
over 50,000 products plus the 53 MB substring cache. The index itself lives in
SQLite's C layer, which `tracemalloc` cannot see, so reporting the Python figure
alone would understate the real cost substantially.

**The 35 s startup is a one-off.** It is index construction plus cache building,
paid once per process rather than per session or per turn. Amortised over 200
sessions it is 174 ms each; over the 800 private sessions it would be 43 ms each.
